...
public class S3UploaderService {
	...
	public FileUploadResponse updateProfile(String userEmail, MultipartFile newProfile, String dirName)
			throws IOException {
		// 현재 사용자의 프로필 정보 조회
		...
		if (user.getUserProfile() != null) {
			currentFileName = extractFileNameFromUrl(user.getUserProfile());
		}

		// 기존 파일 삭제 (프로필 이미지 정보가 없을 경우 삭제하지 않음)
	...
	// 게시물 멀티파일 파일로 변환
	public FileUploadResponse boardUpload(String userEmail, MultipartFile multipartFile, String dirname)
			throws IOException {
		File uploadFile = convert(multipartFile)
				.orElseThrow(() -> new IllegalArgumentException("MultipartFile -> File로 전환이 실패했습니다."));
	...
	// 게시물 파일 수정
	public FileUploadResponse updateBoardProfile(Long bdIdx, MultipartFile newProfile, String dirName)
			throws IOException {
		// 현재 사용자의 프로필 정보 조회
